...
public class DeviceMgmt {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < phones.length; i++) {
            phones[i] = new Phone(in.nextInt(),in.next(),in.next(),in.nextInt());
        }
    ...
    static int findPriceForGivenBrand(Phone[] phones,String brand){
        ...
        for(Phone phone:phones){
            if(phone.getBrand().toLowerCase().equalsIgnoreCase(brand)){
                sum += phone.getPrice();
            }
        }
        return sum;
    ...
    static Phone getPhoneIdBasedOnOs(Phone[] phones,String os){
        for(Phone phone:phones){
            if(phone.getOs().toLowerCase().equalsIgnoreCase(os)){
